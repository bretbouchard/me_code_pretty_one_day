.PHONY: help download train serve generate generate-no-rag benchmark seed test test-integration format-data scrape scrape-local clean

help: ## Show available targets
	@grep -E '^[a-zA-Z_-]+:.*##' Makefile | sed 's/:.*## /\t/' | column -ts $$'\t'

download: ## Download base model (~4GB, requires mlx-lm)
...
generate: ## Generate code with RAG (optional, requires Confucius CLI)
...
seed: ## Seed domain patterns into Confucius (optional, requires Confucius CLI)
...
scrape: ## Scrape corpus from open-source GitHub repos
	python scripts/scrape_corpus.py --domains swift,python

scrape-local: ## Scrape YOUR local repos for training data
	python scripts/scrape_local.py --repos $(REPOS)
